// main.ts - Deno Deploy Entry Point (Light UI + Patgaduu Rebrand + Custom Admin Path)

const kv = await Deno.openKv();

// ============== CONFIGURATION & ADMIN ==============
const ADMIN_PATH = Deno.env.get("ADMIN_PATH") || "/romeo@183110@09688171999";
const ADMIN_API_PATH = ADMIN_PATH + "/api/config";
...
// ============== ADMIN HTML PAGE (LIGHT THEME) ==============

function getAdminHTML(): string {
  const nonce = generateNonce();
  // Admin API path is dynamic based on ADMIN_PATH variable
  const apiPath = escapeHtml(ADMIN_API_PATH);
  return `<!DOCTYPE html>
<html lang="my">
<head>
  <meta charset="UTF-8">
  ...
  <style nonce="${nonce}">
    *{margin:0;padding:0;box-sizing:border-box}
    body{background:#f0f4f8;color:#1e293b;font-family:'Inter','Padauk',system-ui,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}
    .card{max-width:560px;width:100%;background:#ffffff;padding:36px 32px;border-radius:20px;border:1px solid #e2e8f0;box-shadow:0 4px 24px rgba(0,0,0,0.06)}
    h2{text-align:center;font-size:22px;font-weight:800;color:#6d28d9;margin-bottom:6px}
    .subtitle{text-align:center;font-size:12px;color:#94a3b8;margin-bottom:28px;letter-spacing:0.5px}
    label{display:block;margin-top:18px;font-size:12px;font-weight:700;color:#64748b;letter-spacing:0.5px;text-transform:uppercase}
    input,textarea{width:100%;padding:12px 14px;margin-top:8px;background:#f8fafc;border:2px solid #e2e8f0;color:#1e293b;border-radius:12px;font-family:inherit;font-size:14px;transition:all 0.3s}
    input:focus,textarea:focus{outline:none;border-color:#8b5cf6;box-shadow:0 0 0 3px rgba(139,92,246,0.1);background:#fff}
    textarea{resize:vertical}
    button[type="submit"]{margin-top:28px;width:100%;padding:14px;background:#7c3aed;color:#fff;font-weight:700;border:none;border-radius:12px;cursor:pointer;font-size:15px;transition:all 0.3s;letter-spacing:0.3px}
    button[type="submit"]:hover{background:#6d28d9;transform:translateY(-2px);box-shadow:0 8px 24px rgba(124,58,237,0.25)}
...
<body>
  <div class="card">
    <h2>Patgaduu Admin</h2>
    <div class="subtitle">Settings & Configuration Panel</div>
    <form id="adminForm">
  ...
  <script nonce="${nonce}">
    ...
    document.getElementById('adminForm').onsubmit=function(e){
      ...
      fetch(apiUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        ...
        if(r.ok){m.style.color='#16a34a';m.textContent='Saved successfully!';}
        else{r.json().then(function(d){m.style.color='#dc2626';m.textContent=d.error||'Error saving';});}
  ...
  <style nonce="${nonce}">
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .bg-pattern {
      ...
      background-image:
        radial-gradient(ellipse 800px 600px at 10% 0%, rgba(124,58,237,0.05), transparent),
        radial-gradient(ellipse 600px 500px at 90% 100%, rgba(8,145,178,0.04), transparent);
    }
    ...
    .brand-logo svg, .brand-logo img { width: 22px; height: 22px; color: white; position: relative; z-index: 1; }
    ...
    .btn-tg svg { width: 13px; height: 13px; }
    ...
    .banner.active .banner-title { color: var(--success); }
    .banner.expired .banner-title { color: var(--danger); }
    ...
    .stat:nth-child(1)::before { background: linear-gradient(90deg, #7c3aed, #a855f7); }
    .stat:nth-child(2)::before { background: linear-gradient(90deg, #0891b2, #06b6d4); }
    .stat:nth-child(3)::before { background: linear-gradient(90deg, #d97706, #f59e0b); }
    ...
    .stat:nth-child(1) .stat-icon { background: var(--primary-light); color: var(--primary); }
    .stat:nth-child(2) .stat-icon { background: var(--accent-light); color: var(--accent); }
    .stat:nth-child(3) .stat-icon { background: var(--warning-bg); color: var(--warning); }
    ...
    .hero-icon svg { color: white; width: 34px; height: 34px; position: relative; z-index: 1; }
    ...
    .compat-warn svg { width: 16px; height: 16px; flex-shrink: 0; color: var(--danger); }
    .compat-warn strong { color: var(--danger); }
    ...
    .result-expire svg { width: 15px; height: 15px; }
    ...
    .info-row:hover { border-color: #cbd5e1; }
    ...
    .info-row .ir-label svg { width: 17px; height: 17px; color: var(--text-muted); }
    ...
    .info-row.total .ir-value { color: var(--success); }
    ...
    .howto-header:hover { background: #f1f5f9; }
    ...
    .howto-header .ht-label svg { width: 17px; height: 17px; color: var(--text-muted); }
    .howto-header .ht-arrow { color: var(--text-muted); transition: transform 0.3s; }
    .howto-header .ht-arrow svg { width: 17px; height: 17px; }
    ...
    .tg-bar-icon svg { width: 18px; height: 18px; }

    .tg-bar-text { font-size: 12px; color: var(--text-muted); }
    .tg-bar-text strong { display: block; color: var(--accent); font-size: 13px; }
    ...
    .modal-icon.success { background: var(--success-bg); border: 2px solid var(--success-border); }
    .modal-icon.success svg { color: var(--success); width: 30px; height: 30px; }

    .modal h3 { color: var(--text); margin-bottom: 6px; font-size: 18px; }
    ...
    @media (max-width: 420px) {
      .app { padding: 10px; }
      .main { padding: 24px 18px 22px; border-radius: var(--radius); }
      ...
      <div class="howto">
        ...
        <div class="howto-body" id="howtoBody">
          <div class="howto-steps">
            ...
            <div class="step">
              <div class="step-n" style="background:var(--danger-bg);color:var(--danger);">!</div>
              <div style="color:var(--danger);"><strong>Hiddify App</strong> တွင် ဤ Key ကို သုံး၍ <strong>မရနိုင်ပါ</strong>။</div>
            </div>
          </div>
        </div>
      </div>
  ...
  <script nonce="${nonce}">
    ...
    function showQR(){
      ...
      if(typeof qrcode==='undefined'){
        c.innerHTML='<p style="color:#64748b;font-size:12px;padding:20px;">QR Library load မရပါ။ Copy ယူ၍ အသုံးပြုပါ။</p>';
      ...
      try{
        ...
        c.innerHTML='<p style="color:#64748b;font-size:12px;padding:20px;">Key ရှည်လွန်းသဖြင့် QR ဖန်တီးမရပါ။</p>';
      }
      modal.classList.add('show');
    }
...
Deno.serve(async (req) => {
  ...
  // Admin Routes (dynamic path from env variable)
  if (url.pathname === ADMIN_API_PATH) return await handleAdminAPI(req);
